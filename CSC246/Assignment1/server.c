...
/* Set of words maintaint execute;
void trap(int signal){ execute = 0; }ined by the server. */
Word wordList[ MAX_WORDS ];

/* Current number of words in the server's set. */
int wordCount = 0;

// Trap for program exit
int execute;
...
int main( int argc, char *argv[] ) {
  ...
  // Signal trap found here: https://stackoverflow.com/questions/3189650
  // /execute-c-program-till-ctrlc-hit-in-the-terminal
  signal(SIGINT, &trap);
  execute = 1;
