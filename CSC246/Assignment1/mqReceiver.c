// Create a message queue and read then print a string from it.
...
int main() {
  ...
  mqd_t myQueue = mq_open( "/sample-message-queue", O_RDONLY | O_CREAT, 0600, &attr );
  if ( myQueue == -1 )
  ...
  if ( len >= 0 ) {
    ...
    // We just have to print the characters one at a time.
    for ( int i = 0; i < len; i++ )
      printf( "%c", buffer[ i ] );
  } else
  ...
  mq_close( myQueue );
  mq_unlink( "/sample-message-queue" ); // Really, we should use a constant here.
